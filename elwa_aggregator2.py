# elwa_aggregate_csv.py  — thermal-1 対応・未受信時は False で記録
import os
import csv
import json
import time
...
# Thermal デバイスID（送信側に合わせる）
THERMAL_DEV_ID = "thermal_1"
...
TOPIC_MAP = [
    ...
    # ★ DEV_ID を thermal_1 に修正
    (f"/server/{CID}/{THERMAL_DEV_ID}/properties/lepton_occupied", "lepton_occupied", "thermal-1"),
]
        ...
        if now - last_write >= WRITE_EVERY_SEC:
            with latest_values_lock:
                ...
                for k in FIELDNAMES:
                    ...
                    # ★ thermal-1 は未受信の秒でも False を入れる
                    if k == "thermal-1" and v is None:
                        v = False
                    row[k] = v
            try:
