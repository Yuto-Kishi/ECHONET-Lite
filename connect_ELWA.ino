...
void loop() {
  ...
  if (millis() - lastSensorReadTime >= sensorReadInterval) {
    ...
    if (error || co2 < 400 || co2 > 10000) {
      Serial.println("Sensor read error or invalid CO2 data.");
    } else {
      ...
      // 気圧（Pa → hPa に変換）
      float pressure_pa, temperature_dps;
      if (DpsSensor.measurePressureOnce(pressure_pa) == 0) {
        ...
        pressure_doc["pressure"] = pressure_hpa;
